            ...
            switch (message.action) {
                case "init":
                    if (message.session && common.sessions[message.session]) {
                        console.log(
                            "Session ids consistent, websocket initialization for session " +
                                message.session
                        );
                ...
                default:
                    console.log(
                        "Unknown action sent from websocket: " + message.action
                    );
            }
        } catch (ex) {
...
mongodb.MongoClient.connect(
    ...
    function(err, conn) {
        if (err) {
            console.error(
                "Connection to " + config.db + " failed: " + err.name
            );
            process.exit(2);
        }
        ...
        common.templates = db.collection("templates");
